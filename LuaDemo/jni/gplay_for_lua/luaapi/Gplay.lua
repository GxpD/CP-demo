...
function GplayImplement:backFromResourceBundle( group_name)
    print("===> GplayImplement:backFromResourceBundle " .. group_name)
    if luab_gplay:isDivideResourceMode() then
        if type(group_name) == "string" then
            luab_gplay:noticePreloadEnd(group_name)
        else
...
function GplayImplement:preloadResourceBundles(bundleNames, successCallbackFunc)
    ...
    local strJSONGroups = json.encode(jsonGroups)

    gplay._preloadCallback = successCallbackFunc

    luab_gplay:preloadGroups(strJSONGroups, gplay.onPreloadSuccess)
end
