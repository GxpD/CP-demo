...
#define KEYCODE_DPAD_CENTER  0x17
    ...
    {
        LOGE("RTPreloadResponse should not be invoke on Cocos2d-x 3.x");
    }
#else 
// cocos2dx 2.x ================================================================
    void RTPreloadResponse(const char* responseJson, bool isDone, long ext)
    {
        LOGE("RTPreloadResponse should not be invoke on Cocos2d-x 2.x");
    }
...
//------------------------------------------------------------------------------
    void RTPreloadResponse(int resultCode, int errorCode, const std::string& groupName, float percent, float downloadSpeed)
    {
        if (s_preloadResponseCallback)
        {
            ...
            if (resultCode == common::PRELOAD_RESULT_SUCCESS)
                s_preloadSuccessCallback(0, "success");
            return;
        }

        GPlayPreloadLayer* preloadLayer = GPlayPreloadLayer::getCurrInstance();

        if(common::PRELOAD_RESULT_SUCCESS != resultCode)
        {
            if (preloadLayer == NULL)
            {
                preloadLayer = GPlayPreloadLayer::getInstance();
#if COCOS2D_VERSION >= 0x00030000
            ...
            if (resultCode == common::PRELOAD_RESULT_FAILED)
                preloadLayer->onPreloadFailed(errorCode);
            else
                preloadLayer->onUpdateProgress(percent, downloadSpeed);
        } else {
            if (preloadLayer)
