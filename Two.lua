...
function Two:createLayer()
	...
	local layer = cc.Layer:create()
	 gplay:preloadGroup("twoSC", function ()
		-- body
	local bg = cc.Sprite:create("he.jpg")
	bg:setPosition(display.center)
	bg:setTextureRect(CCRectMake(400, 400, 400, 400))
	...
	local button4 = cc.MenuItemFont:create("创建快捷")
                                   :setPosition(display.cx-270, display.cy - 550)
    local button5 = cc.MenuItemFont:create("分享")
                                   :setPosition(display.cx-900, display.cy - 550) 
    local button6 = cc.MenuItemFont:create("绑定c++")
                                   :setPosition(display.cx-100, display.cy - 550) 
...
local function bind()
	...
	--绑定c++方法
    local i = sum(80)
    print("lua_bind_sum: " .. tostring(i)) 
    local s = sd(10)
    print("lua_bind_get: " .. s)
end
	button6:registerScriptTapHandler(bind)
     local mn = cc.Menu:create(button,button1,button2,button3,button4,button5,button6) 
	layer:addChild(mn)
	layer:addChild(bg)
	end)
	return layer
